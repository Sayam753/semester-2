...
void check_again(char s[],int length,trie_node *temp,int flag)
{
  if (temp->is_end==1)
  {
    flag=1;
  ...
  for (int i=0;i<26;i++)
  {
    if (temp->children_array[i]!=NULL)
    {
      ...
      printf("%c",i+'a');
      g_buffer[length_of_buffer]=i+'a';
      length_of_buffer++;
      //print();
      check_again(g_buffer,length_of_buffer,temp->children_array[i],flag+1);
    }
  }
  return;
}
...
void auto_complete(char string[],int length)
{
  int flag=1;
  trie_node *temp  = head_of_trie;
  for (int i=0;i<length;i++)
  {
    temp = temp->children_array[string[i]-'a'];
  }
  if (temp->is_end==1)
  {
    printf("%s",string);
  }
  check_again(string,strlen(string),temp,flag);
}

int isLeafNode(trie_node* root)
{
    return (root->is_end != 0);
}
...
void display(trie_node *root, char str[], int level)
{
    ...
    for (i = 0; i < 26; i++)
    {
        if (root->children_array[i])
        {
            str[level] = i + 'a';
            display(root->children_array[i], str, level + 1);
        }
    }
}

void auto_correct(char s[])
{
  int flag=1,set=1,location=0;
  trie_node *temp = head_of_trie;
  ...
  while(1)
  {
    ...
    switch(choice)
    {
      ...
      case 8:
        ...
        scanf("%s",string);
        auto_complete(string,strlen(string));
        break;
      ...
      case 10:
        printf("Printing all word:\n");
        char str[20];
        display(head_of_trie,str,0);
        break;
    }
  }
}
